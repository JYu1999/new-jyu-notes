...
<form method="POST" action="{{ route('admin.categories.store') }}" id="new-cat-form" class="hidden mb-6 bg-card border border-line rounded-md p-5 space-y-4">
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        <div>
            <label class="block text-xs text-ink-3 mb-1 font-mono uppercase">封面圖路徑</label>
            <input type="text" name="cover_image_path" placeholder="imports/cover.png"
                class="w-full bg-paper border border-line rounded px-3 py-1.5 text-sm font-mono">
        </div>
        <div>
    ...
    @forelse($categories as $cat)
        <div x-data="{ editing: false }" class="bg-card border border-line rounded-md p-5">
            ...
            <form x-show="editing" x-cloak method="POST" action="{{ route('admin.categories.update', $cat) }}" class="space-y-3">
                @csrf @method('PUT')
                <input type="text" name="cover_image_path" value="{{ $cat->cover_image_path }}" placeholder="封面 path"
                    class="w-full bg-paper border border-line rounded px-2 py-1 text-sm font-mono">
