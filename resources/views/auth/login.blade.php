...
@section('title', __('public.login_to_admin'))

@section('content')
<div class="bg-card border border-line rounded-lg p-8 shadow-sm">
    <h1 class="font-serif text-xl font-semibold mb-6 text-center">{{ __('public.login_to_admin') }}</h1>
    ...
    <form method="POST" action="{{ route('auth.login.store') }}" class="space-y-4">
        @csrf
        <div>
            <label for="email" class="block text-sm text-ink-2 mb-1.5">{{ __('public.login_email') }}</label>
        ...
        <div>
            <label for="password" class="block text-sm text-ink-2 mb-1.5">{{ __('public.login_password') }}</label>
            <input id="password" type="password" name="password" required
                class="w-full px-3 py-2 bg-paper border border-line rounded-md focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent-soft">
        </div>
        <label class="flex items-center gap-2 text-sm text-ink-2">
            <input type="checkbox" name="remember" value="1"> {{ __('public.login_remember') }}
        </label>
        <button type="submit" class="w-full bg-accent text-white py-2.5 rounded-md hover:bg-accent-ink font-medium">
            {{ __('public.login_submit') }}
        </button>
    </form>
</div>
<p class="mt-6 text-center text-xs text-ink-3 font-mono">
    <a href="/" class="hover:text-accent">{{ __('public.back_home') }}</a>
</p>
@endsection
